...
export function getSeasonsQueryOptions(leagueId: string) {
  return {
    queryFn: () =>
...
export function getLeagueQueryOptions(leagueId: string) {
  return {
...
function SeasonsTable({ leagueId }: { leagueId: string }) {
  return (
    <Suspense
      fallback={<div className="text-muted-foreground">Loading seasons...</div>}
    >
...
function LeagueDetail() {
  ...
  const isSeasonDetail = pathname.startsWith(`/leagues/${leagueId}/seasons/`)

  return (
    <div className="flex flex-col gap-6">
      <div className={isSeasonDetail ? "hidden" : "flex flex-col gap-6"}>
        <div>
      ...
      <div className={isSeasonDetail ? "block" : "hidden"}>
        <Outlet />
      </div>
    </div>
  )
